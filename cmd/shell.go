		...
		if strings.HasPrefix(line, ".") {
			...
			case strings.HasPrefix(cmd, ".alter_table") || strings.HasPrefix(cmd, ".alter"):
				...
				if ctx.nameserverName != "" {
					...
					fmt.Printf("   CREATE TABLE conversations_%s_new AS SELECT * FROM conversations_%s;\n", ctx.nameserverName, ctx.nameserverName)
					fmt.Println("   -- Step 2: Drop old table")
					fmt.Printf("   DROP TABLE conversations_%s;\n", ctx.nameserverName)
					fmt.Println("   -- Step 3: Rename new table")
...
func printExamples() {
	...
	fmt.Println("    );")
	fmt.Println("    INSERT INTO conversations_name1_new SELECT id, server_id, CAST(priority AS INTEGER), created_at")
